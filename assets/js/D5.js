...
cars.push({
  ...
  model: "Panda",
  color: "red",
  trims: ["Cross", "Mild Hybrid", "Pop"],
});

console.log(cars);

for (i = 0; i < cars.length; i++) {
  const last = cars[i].trims.pop();
  console.log(last);
}
...
for (i = 0; i < cars.length; i++) {
  const first = cars[i].trims.shift();
  console.log(first);
...
let index = 0; // metto index perchè i mi fa saltare tutto il foglio non capisco perchè
